...
bool types_are_equal(Type *t1, Type *t2) {
    ...
    match (kind) {
        case (TYPE_array) return types_are_equal(t1->array_data.type, t2->array_data.type);
        case (TYPE_struct) return String_equal(t1->struct_data.name, t2->struct_data.name);
        default () return true;
    }
}
...
StringRef type_get_name(Type *type) {
    match (type->kind) {
        ...
        default () return StringRef(type_kind_names[type->kind]);
    }
}
